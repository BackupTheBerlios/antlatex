...
\item Erweiterungen wie \code{figbib} oder \code{gloss} werden unterstüzt.
\end{itemize}
...
\code{latexfile} & Die \LaTeX{}-Datei, es werden Dateien mit
  der Endung \code{.tex} oder \code{.ltx} akzeptiert. & Nein, kann durch das Element \code{fileset} gesetzt werden. \\
...
\code{jobname} & Name der Ergebnissdatei & Nein.\\
\code{outputDirectory} & Verzeichnis in dem die DVI/PDF-Datei gespeichert werden soll & Nein.\\
\code{auxDirectory} & Verzeichnis für die temporären Dateien & Nein.\\
...
Die Attribute \code{jobname}, \code{outputDirectory} und
\code{auxDirectory} werden \textbf{nicht} von jeder \LaTeX{}-Version
unterstützt!
...
enthaltenen \code{fileset}-Element können unterschiedliche
Styles wie \code{figbib}\footnote{Bilddatenbank mittels \BibTeX{}}
oder \code{gloss}\footnote{Glossar mittels \BibTeX-Datenbanken}
angebunden werden.
